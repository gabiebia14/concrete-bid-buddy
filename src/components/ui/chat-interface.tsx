...
export function ChatInterface({ clientId, onQuoteRequest, standalone = false }: ChatInterfaceProps) {
  ...
  // Componente interno do chat para reutilização
  const ChatContent = () => (
    <>
      <div className="flex-1 overflow-hidden">
        {messages.length === 0 && !enableBailey ? (
          <div className="flex-1 flex items-center justify-center p-6 text-center h-[400px]">
  ...
  // Renderização para modo standalone (página dedicada)
  if (standalone) {
    return (
      <Layout>
  ...
  // Renderização para modo embutido (dentro de outra página)
  return (
